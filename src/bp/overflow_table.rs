...
impl ChainNode {
    fn new(key: PageKey, value: usize) -> Self {
        Self {
            key,
            value,
            next: Atomic::null(),
...
impl Bucket {
    ...
    fn unlock(&self) {
        ...
        self.version.store((v & !LOCK_BIT).wrapping_add(1), Ordering::Release);
    }
}
        ...
        unsafe {
            ...
            if inlined.is_none() {
                *inlined = Some((key, frame_id));
                ...
                bucket.chain_head.store(new_node, Ordering::Release);
            }
        }
        bucket.unlock();
    }
                ...
                if k == key {
                    ...
                    if !head.is_null() {
                        ...
                        *inlined = Some((node.key, node.value));
                        bucket.chain_head.store(node.next.load(Ordering::Acquire, &guard), Ordering::Release);
